...
//	WM_WORD wPar1, wPar2;
	...
	switch(opt & 0xFF)
	{
...
#endif
			push_DataOfsStack(wDataOfs); //-- by這 wDataOffsetReg
			push_CallingStack(wProgramCounter);
			wDataOfs += wPar1;  //-- by這 wDataOffsetReg
		...
		case VMF_RETFB & 0xFF:		/* RETFB - return from a function block */
			...
			wDataOfs = pull_DataOfsStack(); //-- by這 wDataOffsetReg
			break;
	...
	case VMF_MCD4A & 0xFF:		/* MCD4A - move from code -> data (with 4-byte align ) */
		...
		case VMF_CALP & 0xFF:		
			...
			if (par2)
			{
				...
				wDataOfs = par1; //-- by這 wDataOffsetReg
			}
			}
